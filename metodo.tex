...
%		"Tokenize the file, run physical line checks and yield tokens."}
		...
		\subsection{Projeção e visualização}
			...
			A visualização da projeção pode ser utilizada de duas formas: ativa-se a identificação
	...
	\section{Ferramentas}
		...
		\label{sec:scienceView-Python}
			...
			E a escolha do formato \texttt{CSV}, facilita sua edição em \foreign{softwares}
		...
		A \foreign{ScienceView} estava verificando a similaridade de valores erroneamente,
		...
		\texttt{CSV File (*.csv)}. Depois clique no botão \foreign{Select path} e selecione o
		...
		coleção na base de dados e selecione a coleção, clicando em \foreign{Select} no painel
		\foreign{Selected collection}.
		...
		com o ícone \foreign{Play}. Ou visualizar a projeção a cada instância presente na
		barra de progressão da visualização, localizado na parte inferior da interface ao
		lado dos botões, clicando no botão com o ícone \foreign{Forward}. Ao final, será
		...
		superior, clicando em: \foreign{Data Mining}, \foreign{Topic Extraction and Tracking}
		e \foreign{Run}.
