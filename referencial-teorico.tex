...
\label{chap:Ref}
	...
	\label{sec:Mooc}
		...
		texto presente naquela etapa do ensino e é solicitado a implementação
	...
	\label{sec:FundTeor}
		...
		\label{subsec:similaridade}
			...
			Para isso, deve-se avaliar: a qualidade da projeção quanto % TODO: seria interessante informar os valores para estas projeções. Outra opção é omitir a explicação e a figura, mantendo apenas da segunda frase em diante (como gancho para a próxima subseção).
		...
		\label{subsec:qualidade}
			...
			no conjunto de dados; determinar o número correto de agrupamentos; avaliação
			da análise do agrupamento sem informações externas dos dados; comparação do
			resultado da análise do agrupamento com resultados externos; e a comparação
			de dois agrupamentos distintos para determinar qual é o melhor.
			...
			utilização de uma técnica de projeção multidimensional deve ser a distância
			...
			\subsubsection{Pontuação silhueta}
				...
				melhor sera a média atribuída àquele grupo.
			...
			\label{subsubsec:vizinhanca}
				...
				\cite{paulovich2008hipp}, tem por objetivo verificar as relações de distância
				são mantidas \cite{phd:paulovich}. Para cada objeto multi-dimensional, sua
				vizinhança (\foreign{neighborhood}) é calculada observando os $k$ vizinhos
				mais próximos no plano multidimensional e os $k$ vizinhos mais próximos na
				projeção, sendo $n$ a quantidade de documentos na coleção, $k$ pode variar
				entre $1$ até $n - 1$. Com isso é possível verificar se a proporção de vizinhança
				é preservada (\foreign{preservation}). Seu resultado pode variar entre $0$ e
				$1$. Logo, quanto mais próximo ou igual a $0$, condiz que as vizinhanças de
				um objeto multidimensional e de sua projeção são muito ou totalmente
				distintos. Quanto mais próximo de $1$, melhor é a preservação da vizinhança.
	...
	\label{sec:MinVisual}
		...
		\label{subSec:Caracteristicas}
			...
			As \cref{tab:estEscChamFunc,tab:estEscInstrucao} apresenta possíveis características
	...
	\label{sec:TrabRel}
		...
		as atividades a serem submetidas pode ser a implementação de um programa a
