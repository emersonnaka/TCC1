...
\label{chap:Ref}
	...
	\label{sec:FundTeor}
		...
		\subsection{Dissimilaridade de dados}  % TODO: deixar apenas a subseção de similaridade 
		...
		\label{subsec:similaridade}
			...
			\foreign{Vectors Unit Length} (Parte B). Visualmente, não é possível identificar qual
			a melhor projeção. Para isso, deve-se avaliar: a qualidade da projeção quanto % TODO: seria interessante informar os valores para estas projeções. Outra opção é omitir a explicação e a figura, mantendo apenas da segunda frase em diante (como gancho para a próxima subseção).
			...
			\begin{figure}[h]
				...
				% TODO: pergunta: quão diferente são as figuras se girar a parte B em 180°?
			\end{figure}
		...
		\label{subsec:qualidade}
			...
			\label{subsubsec:vizinhanca}
				...
				(\foreign{neighborhood}) é calculada observando os $k$ vizinhos mais
				...
				proporção de vizinhança é preservada (\foreign{preservation}).Seu resultado
